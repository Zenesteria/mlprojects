...
def count_fingers_raised(rgb_image, detection_result: mp.tasks.vision.HandLandmarkerResult):
   ...
   try:
      ...
      for idx in range(len(hand_landmarks_list)):
         ...
         if(numRaised == 1):
            print('TURN ON ONE LED')
            sio.emit('message', 'ONE')
         elif numRaised == 2:
            print('TURN ON TWO LED')
            sio.emit('message', 'TWO')
         elif numRaised == 3:
            print('TURN ON THREE LED')
            sio.emit('message', 'THREE')
         elif numRaised == 4:
            print('TURN ON FOUR LED')
            sio.emit('message', 'FOUR')
         elif numRaised == 5:
            print('TURN ON FIVE LED')
            sio.emit('message', 'FIVE')
